...
(function () {
  ...
  let originalReplaceState = null; // 保存原始 history.replaceState
   ...
   */
  function generateDarkCSS(theme, performanceMode = false) {
    const {
      backgroundColor,
      textColor,
      secondaryTextColor,
...
${performanceMode
  ? 'div, section, article, main, p, span, li, [class*="content"], [class*="container"]'
  : 'div, section, article, aside, header, footer, main, nav, p, span, li, dt, dd, blockquote, figcaption, label, legend, [class*="container"], [class*="wrapper"], [class*="content"], [class*="card"], [class*="panel"], [class*="box"]'
} {
  background-color: var(--dr-bg) !important;
  color: var(--dr-text) !important;
...
`;
  ...
  function injectFullStyle(theme) {
    ...
    style.textContent = generateDarkCSS(theme, Boolean(currentConfig?.performanceMode));
  ...
  async function loadConfig() {
    const TIMEOUT_MS = 120; // 超过 120ms 视为失败，快速降级，优先防闪白稳定性
    ...
    currentConfig = { mode: 'auto', dimImages: true, ignoreNativeDarkMode: false, performanceMode: false, siteRules: {} };
    return false;
  }
  ...
  function hasNativeDarkHints() {
    try {
      if (document.querySelector('meta[name="color-scheme"][content*="dark" i], meta[name="supported-color-schemes"][content*="dark" i]')) {
        return true;
      }
      const docStyle = window.getComputedStyle(document.documentElement);
      return (docStyle.colorScheme || '').includes('dark');
    } catch (_) {
      return false;
    }
  }
  ...
  function shouldActivate() {
    ...
    if (!currentConfig.ignoreNativeDarkMode && hasNativeDarkHints()) return false;
  ...
  function setupMutationObserver() {
    ...
    mutationObserver = new MutationObserver((mutations) => {
      ...
      const performanceMode = Boolean(currentConfig?.performanceMode);
      // 防抖：性能模式下扩大窗口进一步降低 CPU 占用
      clearTimeout(debounceTimer);
      debounceTimer = setTimeout(() => {
        // 空闲时段处理新增节点，降低主线程抢占
        if (typeof window.requestIdleCallback === 'function') {
          window.requestIdleCallback(() => processAddedNodes(mutations), { timeout: 50 });
        } else {
          processAddedNodes(mutations);
        }
      }, performanceMode ? 24 : 12);
    });
  ...
  function processAddedNodes(mutations) {
    ...
    const performanceMode = Boolean(currentConfig?.performanceMode);
    const childSelector = performanceMode ? 'div,p,span' : 'div,section,article,p,span,li';
    const maxNodesPerBatch = performanceMode ? 30 : 120;
    let processed = 0;

    mutations.forEach(mutation => {
      mutation.addedNodes.forEach(node => {
        if (processed >= maxNodesPerBatch) return;
        ...
        node.querySelectorAll(childSelector).forEach(child => {
          if (processed >= maxNodesPerBatch) return;
          if (isElementSkipped(child)) return;
  ...
  function isElementSkipped(el) {
    if (el.closest && DEFAULT_IGNORED_SELECTORS.some(selector => el.closest(selector))) {
      return true;
    }
